...
# allVariantsAnalysis
######################################################################
allVariantAnalysis <- function( targetdir, figuredir, varclasstype ){
    ...
    classdata <- NULL
    pstats <- NULL
    #for( varclass in c("LOW","MODERATE","HIGH") ){
    for( varclass in 1:5 ){
        tmp <- readClassData( varclass, targetdir, figuredir,varclasstype, patset1,patset2 )
        print(length(tmp))
        classdata <- rbind( classdata, tmp[[1]] )
        print("classdata")
        print(dim(classdata))
...
# Patient stats
    ...
    pstats.tmp <- pstats.annot[pstats.annot$Class %in% c(5,4,3) & pstats.annot$Burden < 1000,]
...
# allVariantsAnalysis
#allVariantAnalysis( targetdir, figuredir, varclasstype, maxclass )
allVariantAnalysis( targetdir, figuredir, varclasstype )
